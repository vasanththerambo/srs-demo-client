  ...
  var finalName = filterName.includes("app")
	...
	useEffect(() => {
		axios.get(Constants.url_getProfiles).then((response) => {
			const tempProfile = response.data.data.find((profile) => profile.email === userEmail);
			if(tempProfile){
				setuserProfileHeader(tempProfile);
	...
	useEffect(() => {
		if (userProfileHeader.image) {
			setprofilePic(userProfileHeader.image);
		}
		else {
				...
				<Dropdown as="li" className="nav-item dropdown header-profile">
					<Dropdown.Toggle variant="" as="a" className="nav-link i-false c-pointer">
						...
						<div className="header-info ms-3">
									  <span>
										  {userEmail || "default@defaultmail.com"}
										  
									  </span>
							<small> {userProfileHeader.accountStatus?"Premium User":"Basic User" }</small>
						</div> 
					</Dropdown.Toggle>
