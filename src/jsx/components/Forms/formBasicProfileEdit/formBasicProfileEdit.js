...
const FormBasicProfileEdit = (props) => {
  ...
  useEffect(()=>{
    if(userProfile.firstName){
        ...
        setbackendPath(`${Constants.url_postProfile}/${userProfile._id}`);
    }
    else{
  ...
  const [lastName, setlasttName] = useState(userProfile.lastName);
  const [email, setemail] = useState(userDetails.email);
  const [gender, setgender] = useState(userProfile.gender);
  const [dob, setdob] = useState(new Date(userProfile.dob));
  const [height, setheight] = useState(userProfile.height);
  const [weight, setweight] = useState(userProfile.weight);
  const [complexion, setcomplexion] = useState(userProfile.complexion);
  const [bodytype, setbodytype] = useState(userProfile.bodytype);
  const [physicalStatus, setphysicalStatus] = useState(userProfile.physicalStatus);
  ...
  const [{ religion, caste }, setData] = useState({religion:userProfile.religion,caste:userProfile.caste}); 
